...
const initialEvents: CalendarEvent[] = [
  ...
  },
  {
    ...
    start: dateToAllDayCalendarDate(
      new Date(new Date().setDate(new Date().getDate() + 2)),
    ),
    end: dateToAllDayCalendarDate(
      new Date(new Date().setDate(new Date().getDate() + 2)),
    ),
    allDay: true,
    category: "personal",
  ...
  start: event.allDay
    ? toCalendarDateTime(event.start as CalendarDate)
    : event.start,
...
export function getEventsForDay(
  ...
  return eventList.filter((event) => {
    ...
    const eventDate =
      event.start instanceof CalendarDateTime ||
      event.start instanceof ZonedDateTime
        ? event.start
