...
import type { Email, EmailFolder, EmailLabel } from "$lib/custom/email/types";
import { mockEmails } from "$lib/custom/email/store/mock-emails";
import { mockLabels } from "$lib/custom/email/store/mock-labels";
...
export const moveToFolder = (id: string, folder: EmailFolder) => {
  emails.update((items) =>
    items.map((email) => (email.id === id ? { ...email, folder } : email)),
  );
};
