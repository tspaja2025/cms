...
{:else if error}
  <div
    class="bg-error-50 border-error-200 text-error-700 rounded-lg border px-4 py-3"
  >
    <p>{error}</p>
    <button
      class="text-error-700 mt-2 font-medium underline"
      onclick={handleBack}
    >
      Go back
    </button>
          ...
          <tbody>
            {#each invoice.items as item, itemIndex (itemIndex)}
              <tr class="border-secondary-100 border-b">
                <td class="text-secondary-800 px-4 py-4 text-sm"
                  >{item.description}</td
                >
                ...
                >
                <td
                  class="text-secondary-800 px-4 py-4 text-right text-sm font-medium"
